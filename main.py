...
async def update_item_category(db_connection, title, new_category):
    # Получаем текущие категории для данного title
...
async def process_element(session, element, sku):
    ...
    try:
        if element.name == 'img':
            # Проверяем, существует ли атрибут 'src'
            if 'src' in element.attrs:
                new_url = await download_img(session, element['src'], sku)
                result_body += " " + new_url
            else:
                # Если 'src' отсутствует, добавляем сообщение об ошибке
                result_body += " "
        ...
        # Обработка исключений, возникших во время выполнения функции
        result_body = " "
        ...
        try:
            ...
            if title in titles:
                if category in categories:
                    continue  # Пропускаем, так как полностью совпадает
                else:
...
async def main():
    ...
    for iss in [["https://www.fashionreps.is/replica-bags/", 10],["https://www.fashionreps.is/replica-accessories/", 2],
                ["https://www.fashionreps.is/replica-jeans-pants/", 4],["https://www.fashionreps.is/replica-nike-x-tiffan/", 2],["https://www.fashionreps.is/brand/dior/", 6],["https://www.fashionreps.is/brand/bape/", 2],
                ["https://www.fashionreps.is/brand/nike/", 19],["https://www.fashionreps.is/brand/prada/", 2],
                ["https://www.fashionreps.is/brand/gucci/", 7],["https://www.fashionreps.is/brand/adidas/", 9],
                ["https://www.fashionreps.is/brand/chanel/", 3],["https://www.fashionreps.is/brand/moncler/", 2],
                ["https://www.fashionreps.is/brand/off-white/", 9],["https://www.fashionreps.is/brand/air-jordan/", 16],
                ["https://www.fashionreps.is/brand/givenchy/", 2],["https://www.fashionreps.is/brand/balenciaga/", 11],
                ["https://www.fashionreps.is/brand/louis-vuitton/", 18],["https://www.fashionreps.is/brand/the/",3],
                ["https://www.fashionreps.is/brand/moose-knuckles/",2],["https://www.fashionreps.is/brand/alexander-mcqueen/",3],
                ["https://www.fashionreps.is/replica-clothing/", 31], ["https://www.fashionreps.is/replica-shoes/", 71]]:
        db_connection = await setup_database()
        async with aiohttp.ClientSession() as session:
            tasks = [process_page(f"{iss[0]}/page/{i}/", session, db_connection,titles,categories) for i in range(1, iss[1])]
